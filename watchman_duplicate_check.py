...
class DuplicateReporter:
    ...
    def find_duplicates(self, computers: List[Dict]) -> Dict[str, List[Dict]]:
        ...
        # Filter and validate groups to only include actual duplicates
        validated_duplicates = {}
        
        for key, computers in mac_os_groups.items():
            if len(computers) > 1:
                # Additional validation: check if these are truly duplicates
                true_duplicates = self._validate_duplicates(computers)
                if len(true_duplicates) > 1:
                    validated_duplicates[key] = true_duplicates
        
        return validated_duplicates
    
    def _validate_duplicates(self, computers: List[Dict]) -> List[Dict]:
        """
        Validate that computers with same MAC+OS are actually duplicates
        by checking serial numbers and other identifying information
        """
        # Group by serial number
        serial_groups = defaultdict(list)
        
        for computer in computers:
            serial = computer.get('serial_number', '').strip()
            
            # Normalize serial number - handle generic/placeholder serials
            normalized_serial = self._normalize_serial(serial)
            
            serial_groups[normalized_serial].append(computer)
        
        # Only computers with the same normalized serial are considered duplicates
        true_duplicates = []
        
        for serial, serial_computers in serial_groups.items():
            if len(serial_computers) > 1:
                # Multiple computers with same MAC, OS, and serial = true duplicates
                true_duplicates.extend(serial_computers)
                
                # Log this validation for transparency
                mac = serial_computers[0].get('system_mac_address', 'Unknown')
                os_type = self._get_os_type(serial_computers[0].get('os_version', ''))
                print(f"ℹ️  Validated {len(serial_computers)} true duplicates: MAC {mac}, OS {os_type.upper()}, Serial {serial}")
            else:
                # Single computer with this serial - likely different physical machine
                # despite same MAC (common with generic network hardware)
                computer = serial_computers[0]
                mac = computer.get('system_mac_address', 'Unknown')
                serial_display = computer.get('serial_number', 'Unknown')
                name = computer.get('computer_name', 'Unknown')
                print(f"ℹ️  Skipping {name}: Same MAC but different serial ({serial_display}) - likely different physical machine")
        
        return true_duplicates
    
    def _normalize_serial(self, serial: str) -> str:
        """
        Normalize serial numbers, handling generic/placeholder values
        """
        if not serial:
            return 'empty_serial'
        ...
        # Handle common generic/placeholder serial numbers
        generic_serials = [
        ...
        # Check if this is a generic serial
        if serial_lower in generic_serials or len(serial_lower.replace('0', '').replace('x', '')) == 0:
            # For generic serials, we need to be more careful
            # Generate a unique identifier based on other computer properties
            return f"generic_serial_{serial_lower}"
        
        # For real serial numbers, normalize case and whitespace
        return serial_lower.replace(' ', '').replace('-', '')
    
    def _handle_generic_serial_duplicates(self, computers: List[Dict]) -> List[Dict]:
        """
        Handle computers that have generic serial numbers but same MAC+OS
        These require additional validation to determine if they're truly duplicates
        """
        # For generic serials, we're more conservative and look at additional factors
        # Group by computer name pattern, creation dates, etc.
        
        # If computer names are very similar (same prefix/pattern), likely duplicates
        # If computer names are completely different, likely separate machines
        
        name_groups = defaultdict(list)
        
        for computer in computers:
            name = computer.get('computer_name', '').strip()
            # Extract name pattern (everything before last number/identifier)
            import re
            name_pattern = re.sub(r'[-_\s]*\d+[-_\s]*[a-zA-Z0-9]*
    ...
    def identify_devices_to_remove(self, duplicate_groups: Dict[str, List[Dict]]) -> List[Tuple[Dict, str]]:
        ...
        for composite_key, computers in duplicate_groups.items():
            ...
            print(f"\n--- Analyzing validated duplicates for MAC: {mac_address} (OS: {os_type.upper()}) ---")
            
            # Show serial numbers for transparency
            print(f"    Serial numbers in group: {', '.join([comp.get('serial_number', 'Unknown') for comp in computers])}")
            ...
            for computer in computers:
                ...
                print(f"  {computer.get('computer_name', 'Unknown')} ({computer.get('client_id', 'Unknown ID')}) - "
                      f"Serial: {computer.get('serial_number', 'Unknown')} - "
            ...
            if len(computer_dates) > 1:
                ...
                for computer, date in to_remove:
                    reason = f"Duplicate MAC {mac_address} with same OS type ({os_type.upper()}) and same/similar serial number, older report date"
                    devices_to_remove.append((computer, reason))
                    print(f"  → REMOVING: {computer.get('computer_name', 'Unknown')} ({computer.get('client_id', 'Unknown ID')}) - Validated duplicate")
...
if __name__ == '__main__':
    main(), '', name).strip()
            ...
            name_groups[name_pattern.lower()].append(computer)
        
        # Only return computers with same name pattern as potential duplicates
        true_duplicates = []
        for pattern, pattern_computers in name_groups.items():
            if len(pattern_computers) > 1:
                true_duplicates.extend(pattern_computers)
                print(f"ℹ️  Generic serial computers with same name pattern '{pattern}': {len(pattern_computers)} potential duplicates")
            else:
                computer = pattern_computers[0]
                print(f"ℹ️  Skipping {computer.get('computer_name', 'Unknown')}: Generic serial but unique name pattern")
